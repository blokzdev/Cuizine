...
**Operating modes.** The three rules above describe the checkpoint *content*; *how* confirmation happens has two modes. In **interactive mode** (the default), the agent stops at each phase gate and the founder confirms synchronously. In **delegated mode**, the founder grants a standing delegation — itself a founder decision, made in advance and revocable at any time, recorded as entry #0 in `DECISION-LOG.md` — authorizing the agent to resolve checkpoints autonomously inside a long-running build loop: `[research-informed]` checkpoints resolve by a keep-unless-disqualified rule (keep the documented choice unless research reveals deprecation/abandonment, a security advisory, a breaking change, or clear incompatibility; otherwise take the minimal-deviation alternative), `[data-driven]` checkpoints resolve by logged in-build observation at the named moment, and alpha-contingent ones are explicitly deferred. Every resolution is logged in `DECISION-LOG.md`; changed *product* decisions still produce superseding ADRs per Rule 2; and founder review moves from synchronous gates to asynchronous reading of pushed phase reports, with steering via `FOUNDER-FEEDBACK.md`. Delegated mode does not weaken Rule 1 — the founder still decides; the decision is exercised in advance through the delegation's rules rather than at each gate. Mode selection is an operational choice made at build kickoff and recorded in the ledger, not a foundation change.
